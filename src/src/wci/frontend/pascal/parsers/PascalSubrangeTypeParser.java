...
public class PascalSubrangeTypeParser extends PascalTypeSpecificationParser
{
    /**
     * Constructor.
     * @param parent the parent parser.
     */
    protected PascalSubrangeTypeParser(PascalParserTD parent)
    {
        super(parent);
    }
        ...
        throws Exception
    {
        ...
        PascalConstantDefinitionsParser constantParser =
        ...
        if (PascalConstantDefinitionsParser.CONSTANT_START_SET.contains(tokenType)) {
            if (!sawDotDot) {
                errorHandler.flag(token, MISSING_DOT_DOT, this);
            }

            // Parse the maximum constant.
            token = synchronize(PascalConstantDefinitionsParser.CONSTANT_START_SET);
            constantToken = token;
